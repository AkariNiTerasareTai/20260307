...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeLightbox = () => {
        ...
        lightboxImg.style.transform = ''; // スワイプ位置をリセット
        setTimeout(() => {
    ...
    // どこを触っても（背景でも画像でも）閉じる
    lightbox.addEventListener('pointerdown', (e) => {
        startY = e.clientY;
        isDragging = true;
        lightboxImg.style.transition = 'none'; // ドラッグ中はアニメーションを切る
    });
    ...
    window.addEventListener('pointermove', (e) => {
        ...
        if (deltaY > 0) {
            ...
            lightbox.style.opacity = Math.max(0, 1 - deltaY / 400); // 徐々に薄くする
        }
    });
    ...
    window.addEventListener('pointerup', (e) => {
        ...
        if (deltaY > 100) {
            ...
            // それ以外なら、単なるタップ（移動が少ない）かスワイプ不足なので元に戻すか閉じる
            if (Math.abs(deltaY) < 5) {
                // 移動がほとんどなければタップとみなして閉じる（どこでもタップクローズ）
                closeLightbox();
            } else {
                // スワイプが足りなければ元の位置に復帰
                lightboxImg.style.transform = 'translateY(0)';
